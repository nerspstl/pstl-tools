...
def choose_port(list_resources,open_resource,resource_kws={}):
    """
...
class Open():
    def __init__(self,list_resources,open_resource,port=None,**kwargs):
        ...
        if port is not None:
            try:
                res=open_port(port,open_resource,kwargs)
            except:
                print("\nFailed to open %s"%(port))
                port=None
        if port is None:
            res=choose_port(list_resources,open_resource,kwargs)
